...
## トラブルシューティング: 年間メモの「支払い日」が `2026-07-27T15:00:00.000Z` のような表示になる

年間メモ（自動車税・固定資産税など）の「支払い日」欄に `2026-07-28` のような日付として解釈できる文字列を入力すると、上記と同じ理由でスプレッドシートが自動的に日付型に変換してしまうことがあります。

`gas/Code.gs` は最新版で読み込み時に自動整形（`M月D日` 形式に変換）するようになっているため、**再デプロイするだけで表示が直ります**（データの再入力は不要）。あわせて、書き込み時にも `AnnualMemos` シートの `payment_date` 列（C列）を常にプレーンテキスト書式にする対策をしているので、再デプロイ後の新規追加・編集ではこの問題は発生しません。

再デプロイの手順は上記「入力したのに総資産推移にしか反映されない」の 1〜2 と同じです。念のためスプレッドシート上のセルの値自体もテキスト化しておきたい場合は、メニューの **資産管理 > 年月データの修復（テキスト化）** を実行してください（`MonthlyBalances` / `ElectricityRecords` の年月列とあわせて `AnnualMemos` の `payment_date` 列も一括修復されます）。
