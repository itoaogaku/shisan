...
`gas/Code.gs` は年月セルを常にプレーンテキストとして書き込み・読み込む対策済みです。もし発生した場合は、**Apps Script を最新版として再デプロイ**（「デプロイ > デプロイを管理 > 編集 > 新バージョン」）した上で、該当月のデータをもう一度保存し直してください（保存し直した行から正しいテキスト形式に修正されます）。
